...
const newPossibleLetters = () => ({ spaces: Array(WORD_LENGTH).fill().map((_, i) => ({ blacklist: [] })), letterCounts: {} });

if (words.some(word => word.length !== WORD_LENGTH)) throw `Not all words are ${WORD_LENGTH} letters long!`;
...
const playGame = () => {
    ...
    while (true) {
        ...
        const guessOutput = prompt('What was output? ').split(',').map(a => +a);
        possibleLetters = parseGuess(guess, guessOutput, possibleLetters);
...
const V = (possibleLetters) => {
    ...
    const usefulWords = words.filter(word => isWordUseful(word, possibleLetters))
...
const parseGuess = (guess, guessOutput, oldPossibleLetters) => {
    ...
    const letterCountsInGuess = guess.split('').reduce((p, c) => ({ ...p, [c]: (p[c] || 0) + 1 }), {});
    ...
    guessOutput.forEach((result, i) => {
        ...
        if (result === 2) {
            ...
            letterCountsInAnswer[letter] = (letterCountsInAnswer[letter] || 0) + 1;
            return;
        }
        if (result === 1) {
    ...
    Object.keys(letterCountsInGuess).forEach(letter => {
        ...
        if (!possibleLetters.letterCounts[letter]) possibleLetters.letterCounts[letter] = [0, 5];
        possibleLetters.letterCounts[letter][0] = Math.max(possibleLetters.letterCounts[letter][0], letterCountsInAnswer[letter]);
        if (letterCountsInAnswer[letter] < letterCountsInGuess[letter]) possibleLetters.letterCounts[letter][1] = letterCountsInAnswer[letter];
    });
...
const filterWords = (words, possibleLetters) => {
    return words.filter(word => {
        if (word.split('').some((letter, i) => {
            if (possibleLetters.spaces[i].letter) return possibleLetters.spaces[i].letter !== letter;
            return possibleLetters.spaces[i].blacklist.includes(letter);
        })) return false;

        const counts = word.split('').reduce((p, c) => ({ ...p, [c]: (p[c] || 0) + 1 }), {});

        const
        return Object.keys(counts).every(letter => !possibleLetters.letterCounts[letter] || (counts[letter] >= possibleLetters.letterCounts[letter][0] && counts[letter] <= possibleLetters.letterCounts[letter][1]));
    });
}
...
const isWordUseful = (word, possibleLetters) => {
    ...
    if (Object.keys(counts).some(letter => !possibleLetters.letterCounts[letter] || (counts[letter] > possibleLetters.letterCounts[letter][0] && counts[letter] <= possibleLetters.letterCounts[letter][1]))) return true;
...
const deepCopy = (object) => {
    if (typeof object === 'object') {
        if (object.length === undefined) return Object.keys(object).reduce((p, key) => ({ ...p, [key]: deepCopy(object[key]) }), {});
