...
const update = () => {
    for (const body of bodies) {
        // console.log(body);
        const area = body.vertices
            .slice(1, body.vertices.length - 1)
            .reduce((p, vertex, i) => {
        ...
        const outwardVectors = body.vertices.map((vertex, i) => {
            const v1 = unitVec(
                subVec(
                    vertex.pos,
                    body.vertices[
                        (body.vertices.length + i - 1) % body.vertices.length
                    ].pos
                )
            );
            const v2 = unitVec(
        ...
        for (const [i, vertex] of body.vertices.entries()) {
            ...
            if (nextLen > 0)
                vertex.v = addVec(
                    vertex.v,
                    multConstVec(
                        (body.sideLengths[i] / nextLen - 1) * body.sideK,
                        nextDiff
                    )
                );
            ...
            if (lastLen > 0)
                vertex.v = addVec(
                    vertex.v,
                    multConstVec(
                        (body.sideLengths[last] / lastLen - 1) * body.sideK,
                        lastDiff
                    )
                );
            ...
            vertex.v = addVec(
                vertex.v,
                multConstVec(deltaArea * body.areaK, outwardVectors[i][0])
            );

            // Smoothness spring
            vertex.v = addVec(
                vertex.v,
                multConstVec(-body.smoothnessK / 2, outwardVectors[i][1])
            );
...
const newBody = (
    ...
    return {
        ...
        areaK,
        smoothnessK,
        vertices: Array(sideLengths.length)
            .fill()
            .map(() => ({
                pos: addVec(startPos, [Math.random() * 50, Math.random() * 50]),
                v: [0, 0],
            })),
    };
};
...
const posInBody = (body, pos) => {
    for (const [i, vertex] of body.vertices
        .slice(1, body.vertices.length - 1)
        .entries()) {
        ...
        let crosses = 0;
        if (
            inRange(a[0], b[0], pos[0]) &&
            a[1] + ((pos[0] - a[0]) * (b[1] - a[1])) / (b[0] - a[0]) > pos[1]
        )
            crosses++;
        if (
            inRange(a[0], vertex.pos[0], pos[0]) &&
            a[1] +
                ((pos[0] - a[0]) * (vertex.pos[1] - a[1])) /
                    (vertex.pos[0] - a[0]) >
                pos[1]
        )
            crosses++;
        if (
            inRange(b[0], vertex.pos[0], pos[0]) &&
            b[1] +
                ((pos[0] - b[0]) * (vertex.pos[1] - b[1])) /
                    (vertex.pos[0] - b[0]) >
                pos[1]
        )
            crosses++;
