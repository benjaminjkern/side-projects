const NUMDECKS = 10000;
const DEALER_STAY_AT = 17;

const drawCardProbs = (...existingCards) => {
    const counts = existingCards.reduce((p, c) => ({ ...p, [c]: (p[c] || 0) + 1 }), {});
    const cardsLeft = NUMDECKS * 52 - existingCards.length;
    return Array(10).fill().map((_, i) => i + 1).reduce((p, card) => ({
        ...p, [card]:
            Math.max(0, (NUMDECKS * 4 * (card === 10 ? 4 : 1) - (counts[card] || 0)) / cardsLeft)
        // (card === 10 ? 4 : 1) / 13
    }), {});
}
