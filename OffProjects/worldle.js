...
const newKnowledgebase = (kb) => {
    if (!kb) return {
        letters: Array(WORDLENGTH).fill().map(() => ({ correctLetter: undefined, blacklistLetters: {} })),
        requiredLetters: {},
        blacklistLetters: {},
    };
    return {
        letters: Array(WORDLENGTH).fill().map((_, i) => ({ correctLetter: kb.letters[i].correctLetter, blacklistLetters: { ...kb.letters[i].blacklistLetters } })),
...
const possibleAnswers = (words, kb) => {
    return words.filter(word => {
        if (Object.keys(kb.requiredLetters).some(letter => !word.includes(letter))) return false;
        if (Object.keys(kb.blacklistLetters).some(letter => word.includes(letter))) return false;
        return word.split('').every((letter, i) => kb.letters[i].correctLetter ? kb.letters[i].correctLetter === letter : !kb.letters[i].blacklistLetters[letter]);
    })
};

const valuableGuessAnswers = (words, kb, possible = possibleAnswers(words, kb)) => {
    return [...possible, ...words.filter(guess => {
        return !possible.includes(guess) && possible.some(answer =>
            possibleAnswers(possible, parseMatch(guess, match(guess, answer), kb)).length < possible.length
        )
    })];
};
const VCache = {};
...
const V = (words = WORDS, kb = newKnowledgebase(), possible = possibleAnswers(words, kb)) => {
...
const Q = (guess, words = WORDS, kb = newKnowledgebase(), possible = possibleAnswers(words, kb)) => {
