...
public class MandelbrotFrame extends Keyframe {
    ...
    MandelbrotFrame(int width, int height, double centerX, double centerY, double zoom, double zoomFactor,
            MandelbrotFrame parent) {
    ...
    class Pixel {
        ...
        Pixel(int boardX, int boardY) {
            ...
            startPos = new ComplexNum(((double) boardX / (double) width) * zoom * 2 + centerX - zoom,
                    -(((double) boardY / (double) height) * zoom * 2 - centerY - zoom));
    ...
    public MandelbrotFrame nextFrame() {
        if (next == null)
            next = new MandelbrotFrame(width, height, centerX, centerY, zoom * zoomFactor, zoomFactor, this);
