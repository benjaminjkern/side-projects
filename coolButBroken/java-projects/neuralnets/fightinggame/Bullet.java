...
public class Bullet extends CircObstacle {
    ...
    public Bullet(Character character) {
        ...
        this.speed = new double[] { DEFAULT_SPEED * Math.cos(Math.toRadians(character.angle)),
                DEFAULT_SPEED * Math.sin(Math.toRadians(character.angle)) };
    }

    public Bullet(double x, double y, double speed, double angle) {
        super(x, y, RADIUS);
        this.speed = new double[] { DEFAULT_SPEED * Math.cos(Math.toRadians(angle)),
                DEFAULT_SPEED * Math.sin(Math.toRadians(angle)) };
