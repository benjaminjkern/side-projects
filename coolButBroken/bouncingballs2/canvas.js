...
const move = (ctx) => {
    balls.forEach((ball, i) => {
        ball.x = add(
            add(mult(2, ball.path[0]), mult(-1, ball.path[1])),
            mult(dt ** 2, accel(ball.path[0]))
        );
...
const calcEnergy = () =>
    balls.reduce(
        (p, c) =>
            p +
            0.5 * c.mass * lengthSquared(c.v) -
            c.mass * dot(sub(c.pos, [0, canvas.height]), gravity),
        0
    );
...
const addEventListener = (target, listenerType, func) => {
    if (!target[listenerType]) target[listenerType] = func;
        ...
        target[listenerType] = function (e) {
...
const size = (A) => {
    ...
    if (A.slice(1).every((x) => deepEquals(size(x), first)))
...
const dot = (A, B) => {
    ...
    return A.reduce(
        (p, c, i) => (p ? add(p, mult(c, B[i])) : mult(c, B[i])),
        undefined
    );
};
...
const add = (A, B, validated = false) => {
    ...
    return [
        add(A[0], B[0], validate),
        ...add(A.slice(1), B.slice(1), validate),
    ];
};
...
const print = (A) => {
    console.log(
        A.map((row) =>
            row.map((num) => Math.round(num * 100) / 100).join("\t")
        ).join("\n"),
        "\n"
    );
};
