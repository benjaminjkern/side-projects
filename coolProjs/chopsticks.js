...
const getMoves = (game) => {
    return game[0].reduce((p, stick0, j) => {
        return stick0 === 0 || game[0].slice(j + 1).includes(stick0)
            ? p
            : [
                  ...p,
                  ...game[1]
                      .map((stick1, i) => {
                          if (
                              stick1 === 0 ||
                              game[1].slice(i + 1).includes(stick1)
                          )
                              return [];
                          const newGame = [[...game[0]], [...game[1]]];
...
const relativeScores = (game) =>
    getMoves(game).map((nextGame) => ({
        [makeKey(nextGame)]: -VOpt(nextGame),
    }));

const color = (finger, selected = false) =>
    (finger + "")[finger === 0 ? "gray" : selected ? "bgYellow" : "yellow"];
...
const display = (game, you = true, selection = -1) => {
  ...
  ${you ? ">" : " "}  ${game[0]
        .map((finger, i) => color(finger, selection === i))
        .join("   ")}
`);
};
