...
public class Tools {
     ...
     * concat(a, b, [dim])
     * 
     * Concatenates matrix, vector, or number a and with matrix, vector, or number b together in the dimension specified.
     * 
     * - If dim is not specified it tries to guess dimension by looking at the sizes of a and b.
     * - If a and b are vectors and dim is not specified, it defaults to stacking them on top of each other in to one vector.
     * 
     */
    ...
    public static double[] concat(double a, double[] b) {return flip(concat(flip(b), a));} //might not be super fast but its okay
     ...
     * 
     * - If a is a vector, it is taken to be a column vector and returned as a horizontal vector-matrix.
     * 
     */
...
class ImproperDimensionException extends RuntimeException
{
    ...
    public ImproperDimensionException(double[][] a, double[][] b)
    {
        super(BASIC+" a: ["+a.length+", "+(a.length==0?0:a[0].length)+"], b: ["+b.length+", "+(b.length==0?0:b[0].length)+"]");
    }
}
