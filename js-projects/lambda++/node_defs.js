...
class Assignment {
    ...
    toString(names) {
        return `${
      names[this.id] ? names[this.id] : this.id
    } = ${this.expression.toString(names)}`;
    }
}
...
class Lambda {
    ...
    toString(names) {
        const value = this.detectType();
        return value !== null ?
            value :
            `λ${
          names[this.variable] ? names[this.variable] : this.variable
        }.${this.expression.toString(names)}`;
    }
    detectType() {
        // it is likely a list or a pair if its a raw lambda
        if (!(this.expression instanceof Lambda)) return null;
        // number
        if (!(this.expression.expression instanceof Lambda)) {
            const c = count(
                this.variable,
            ...
            if (!isNaN(c)) {
                if (c === 0) return "(0 or false)";
                return c;
            }

            // boolean, only need to test for true since false actually is the same as 0
            if (
                this.expression.expression instanceof Identifier &&
                this.expression.expression.id === this.variable
            )
                return true;
        }
...
const count = (f, x, exp) => {
    if (exp instanceof Identifier) {
        if (exp.id === x) return 0;
        return NaN;
    }
    if (exp instanceof Application) {
        if (exp.left instanceof Identifier) {
            if (exp.left.id === f) return 1 + count(f, x, exp.right);
            return NaN;
        }
        return NaN;
    }
...
class Identifier {
    ...
    toString(names) {
        return names[this.id] ? names[this.id] : this.id;
    }
}
