...
public class Tools {
    ...
    // ZEROFINDER CODE
    // find all real zeros of any polynomial given as a list of doubles
    // [a,b,c,d,e,...]
    // a + bx + cx^2 + dx^3 + ex^4 + ...
    public static double[] findZeros(double[] coefficients) {
        if (coefficients.length == 1) return new double[] {};
        if (coefficients[coefficients.length - 1] == 0) return findZeros(slice(coefficients, 0, -1));
        if (coefficients.length == 2) return new double[] { -coefficients[0] / coefficients[1] }; // linear
        if (coefficients.length == 3) {
            // probably don't actually need this section but it gives quicker and more
            // accurate results
            double disc = Math.pow(coefficients[1], 2) - 4 * coefficients[0] * coefficients[2];
            if (disc < 0) return new double[] {};
            return new double[] { (-coefficients[1] + Math.sqrt(disc)) / (2 * coefficients[2]),
                    (-coefficients[1] - Math.sqrt(disc)) / (2 * coefficients[2]), };
        }
        if (coefficients.length % 2 == 1) {
            // power is even
            double[] criticalPoints = findZeros(derivative(coefficients));
            // if all critical points are on the same side of zero as the limits to
            // infinity, then there are no more zeros to be found
            for (int i = 0; i < criticalPoints.length; i++) {
                if (polyEval(coefficients, criticalPoints[i]) > 0 != coefficients[coefficients.length - 1] > 0) break;
                if (i == criticalPoints.length - 1) return new double[0];
            }
        }
        // an answer must exist from this point on, so we can use midpoint method
        double max = Double.MAX_VALUE;
        double min = -Double.MAX_VALUE;
        double testX = Double.MAX_VALUE;
        String lastX = "";
        while (lastX.isEmpty() || !(testX + "").equals(lastX)) {
            lastX = testX + "";
            testX = max / 2. + min / 2.;
            double maxY = polyEval(coefficients, max);
            double minY = polyEval(coefficients, min);
            testX = max - (maxY * (max - min)) / (maxY - minY);
            double y = polyEval(coefficients, testX);
            if (y == 0) break;
            if (y > 0 == maxY > minY) max = testX;
            else min = testX;
        }
        return concat(testX, findZeros(divideBy(coefficients, testX)));
    }

    public static double[] derivative(double[] coefficients) {
        double[] newCoefficients = new double[coefficients.length - 1];
        for (int i = 0; i < coefficients.length - 1; i++) { newCoefficients[i] = coefficients[i + 1] * (i + 1); }
    ...
    public static double polyEval(double[] coefficients, double x) {
        ...
        for (int i = coefficients.length - 1; i >= 0; i--) {
            double bit = coefficients[i] * Math.pow(x, i);
            if (bit == Double.POSITIVE_INFINITY) return Double.MAX_VALUE;
            if (bit == Double.NEGATIVE_INFINITY) return -Double.MAX_VALUE;
            sum += bit;
        }
        return sum;
    }

    // note: if the last number is anything other than 0 (or -0) then the zero given
    // was not properly a zero of the equation.
    // Sort of a NP test of checking whether or not it is the right answer
    public static double[] divideBy(double[] coefficients, double zero) {
        if (zero == 0) return slice(coefficients, 1);
        double[] newCoefficients = new double[coefficients.length - 1];
        double b = 0;
        for (int i = 0; i < coefficients.length - 1; i++) {
            b = (b - coefficients[i]) / zero;
            newCoefficients[i] = b;
        }
        return newCoefficients;
    }
    ...
    public static String[] slice(String[] array, int start, int end) {
        ...
        int nend = end >= 0 ? end : end + array.length;
        String[] newArray = new String[nend - start];
        for (int i = start; i < nend; i++) { newArray[i - start] = array[i]; }
        return newArray;
    }

    public static String[] slice(String[] array, int start) {
        if (start == 0) return array;
        String[] newArray = new String[array.length - start];
        for (int i = start; i < array.length; i++) { newArray[i - start] = array[i]; }
        return newArray;
    }
    ...
    public static int[] slice(int[] array, int start, int end) {
        if (start == 0 && end == array.length) return array;
        int nend = end >= 0 ? end : end + array.length;
        int[] newArray = new int[nend - start];
        for (int i = start; i < nend; i++) { newArray[i - start] = array[i]; }
        return newArray;
    }
    ...
    public static double[] slice(double[] array, int start, int end) {
        if (start == 0 && end == array.length) return array;
        int nend = end >= 0 ? end : end + array.length;
        double[] newArray = new double[nend - start];
        for (int i = start; i < nend; i++) { newArray[i - start] = array[i]; }
        return newArray;
    }
    ...
    public static double[] slice(double[] array, int start) {
        if (start == 0) return array;
        double[] newArray = new double[array.length - start];
        for (int i = start; i < array.length; i++) { newArray[i - start] = array[i]; }
        return newArray;
    }
    ...
    public static String print(int[] array) {
        ...
        for (int a = 1; a < array.length; a++) { print.append(", " + array[a]); }
    ...
    public static String print(double[] array) {
        ...
        for (int a = 1; a < array.length; a++) { print.append(", " + Math.round(array[a] * 100) / 100.); }
    ...
    public static String print(Object[] array) {
        ...
        for (int a = 1; a < array.length; a++) { print.append(", " + array[a].toString()); }
    ...
    public static String print(double[][] array) {
        ...
        for (int a = 1; a < array.length; a++) { print.append(", " + print(array[a])); }
    ...
    public static void println(double[] a) { System.out.println(print(a)); }

    public static void println(double[][] a) { System.out.println(print(a)); }

    public static void println(int[][] a) { System.out.println(print(a)); }
    ...
    public static int[] round(double[] a) {
        ...
        for (int o = 0; o < a.length; o++) { output[o] = round(a[o]); }
        return output;
    }
    ...
    public static int floor(double a) { return (int) a; }

    public static int[] floor(double[] a) {
        int[] output = new int[a.length];
        for (int o = 0; o < a.length; o++) { output[o] = floor(a[o]); }
        return output;
    }
    ...
    public static int sgn(double a) { return Math.abs(a) < 1e-12 ? 0 : (int) Math.round(a / Math.abs(a)); }

    public static int[] sgn(double[] a) {
        int[] output = new int[a.length];

        for (int i = 0; i < a.length; i++) { output[i] = sgn(a[i]); }
        return output;
    }
    ...
    public static int[][] sgn(double[][] a) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = sgn(a[y]); }
        return output;
    }

    public static int sgn(int a) { return sgn((double) a); }

    public static int[] sgn(int[] a) { return sgn(a); }

    public static int[][] sgn(int[][] a) { return sgn(a); }

    // sigmoid function
    public static double sigmoid(double a) { return 1. / (1 + Math.exp(-a)); }

    public static double[] sigmoid(double[] a) {
        double[] output = new double[a.length];

        for (int i = 0; i < a.length; i++) { output[i] = sigmoid(a[i]); }
        return output;
    }
    ...
    public static double[][] sigmoid(double[][] a) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = sigmoid(a[y]); }
        return output;
    }
    ...
    public static double[] ReLU(double[] a) {
        ...
        for (int i = 0; i < a.length; i++) { output[i] = ReLU(a[i]); }
        return output;
    }
    ...
    public static double[][] rand(double low, double high, int height, int width) {
        ...
        for (int y = 0; y < height; y++) { output[y] = rand(low, high, width); }
        return output;
    }
    ...
    public static double min(double[] a) {
        ...
        for (int i = 0; i < a.length; i++) { if (a[i] < currentMin) { currentMin = a[i]; } }
        return currentMin;
    }
    ...
    public static double max(double[] a) {
        ...
        for (int i = 0; i < a.length; i++) { if (a[i] < currentMax) { currentMax = a[i]; } }
        return currentMax;
    }
    ...
    public static double[][] zeros(int height, int width) {
        ...
        for (int y = 0; y < height; y++) { output[y] = zeros(width); }
        return output;
    }
    ...
    public static double sum(double[][] a) {
        ...
        for (int y = 0; y < a.length; y++) { output += sum(a[y]); }
        return output;
    }
    ...
    public static double[] sum(double[][] a, int dim) {
        if (dim == 0) {
            double[] output = new double[a.length];
            for (int y = 0; y < a.length; y++) { output[y] = sum(a[y]); }
            return output;
        } else if (dim == 1) { return sum(transpose(a), 0); }

        throw new IllegalArgumentException();
    }

    // average!
    public static double average(double[] a) { return sum(a) / a.length; }

    public static double average(double[][] a) { return sum(a) / (a.length * a[0].length); }
    ...
    public static double[] makeCopy(double[] a) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = a[y]; }
        return output;
    }
    ...
    public static double[][] makeCopy(double[][] a) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = makeCopy(a[y]); }
        return output;
    }
    ...
    public static double[] cPow(double[] a, double b) {
        ...
        for (int i = 0; i < a.length; i++) { output[i] = Math.pow(a[i], b); }

        return output;
    }

    public static double[] cPow(double a, double[] b) { return cPow(b, a); }

    public static double[] pow(double[] a, double b) { return cPow(a, b); }
    ...
    public static double[][] cPow(double[][] a, double b) {
        ...
        for (int i = 0; i < a.length; i++) { output[i] = cPow(a[i], b); }
    ...
    public static double[][] cPow(double[][] a, double[] b) { throw new UnsupportedOperationException(); }
    ...
    public static double[][] matrix(int[][] a) {
        ...
        for (int i = 0; i < a.length; i++) { mat[i] = vector(a[i]); }
        return mat;
    }
    ...
    public static double[][] matrix(double[] a) {
        ...
        for (int i = 0; i < a.length; i++) { mat[i][0] = a[i]; }
        return mat;
    }
    ...
    public static double[] cast2vect(double[][] a) {
        if (a.length == 1) {
            ...
            for (int i = 0; i < a[0].length; i++) { output[i] = a[0][i]; }
            return output;
    ...
    public static double add(double... ds) {
        double sum = 0;
        for (int i = 0; i < ds.length; i++) { sum += ds[i]; }
        return sum;
    }
    ...
    public static double[] add(double[] a, double b) {
        ...
        for (int i = 0; i < a.length; i++) { output[i] = a[i] + b; }
        return output;
    }

    public static double[] add(double a, double[] b) { return add(b, a); }
    ...
    public static double[] add(double[]... ds) {
        double[] output = zeros(ds[0].length);
        for (int i = 0; i < output.length; i++) {
            if (ds[i].length != output.length) throw new ImproperDimensionException(ds);
            output = add(output, ds[i]);
        }
        return output;
    }
    ...
    public static double[][] add(double[][] a, double b) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = add(a[y], b); }
        return output;
    }
    ...
    public static double[][] add(double[][] a, double[][] b) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = add(a[y], b[y]); }
        return output;
    }

    public static double subtract(double a, double b) { return a - b; }

    public static double[] subtract(double[] a, double b) { return add(a, -b); }

    public static double[] subtract(double a, double[] b) { return add(a, mult(-1, b)); }
    ...
    public static double[] cMult(double[] a, double b) { return mult(a, b); }
    ...
    public static double[][] cMult(double[][] a, double[] b) {
        if (a[0].length == b.length) {
            ...
            for (int y = 0; y < a.length; y++) { output[y] = cMult(a[y], b); }
            return output;
        } else if (a.length == b.length) { return transpose(cMult(transpose(a), b)); }

        throw new ImproperDimensionException(a, b);
    }

    public static double[][] cMult(double[] a, double[][] b) { return cMult(b, a); }
    ...
    public static double[][] cMult(double[][] a, double[][] b) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = cMult(a[y], b[y]); }
        return output;
    }
    ...
    public static double[] mult(double a, double[] b) { return mult(b, a); }

    // vector x vector depends on how you want to do it!
    public static double[] mult(double[] a, double[] b) { return cMult(a, b); }
    ...
    public static double[][] mult(double[][] a, double b) {
        ...
        for (int y = 0; y < a.length; y++) { output[y] = mult(a[y], b); }
        return output;
    }
    ...
    public static double[] mult(double[] a, double[][] b) {
        ...
        for (int i = 0; i < b[0].length; i++) { output[i] = dotProduct(a, transpose(b)[i]); }
        return output;
    }
    ...
    public static double[] mult(double[][] a, double[] b) {
        ...
        double[] output = new double[a.length];
        for (int i = 0; i < a.length; i++) { output[i] = dotProduct(a[i], b); }
        return output;
    }
    ...
    public static double[][] mult(double[][] a, double[][] b, boolean innerOuter) {
        ...
        if (innerOuter) {
            ...
            for (int i = 0; i < b.length; i++) { output = add(output, outerProduct(transpose(a)[i], b[i])); }
        }
        return output;
    }
    ...
    public static double dotProduct(double[] a, double[] b) {
        ...
        for (int i = 0; i < a.length; i++) { output += a[i] * b[i]; }
        return output;
    }

    public static double[] wedgeProduct(double[] a, double[] b) { throw new UnsupportedOperationException(); }

    public static double innerProduct(double[] a, double[] b) { return dotProduct(a, b); }

    public static double[][] outerProduct(double[] a, double[] b) {
        double[][] output = new double[a.length][b.length];

        for (int y = 0; y < a.length; y++) { for (int x = 0; x < b.length; x++) { output[y][x] = a[y] * b[x]; } }
        return output;
    }

    public static double det(double[][] a) { throw new UnsupportedOperationException(); }

    public static double[][] inverse(double[][] a) { throw new UnsupportedOperationException(); }
    ...
    public static double[][] slice(double[][] a, int y1, int y2, int x1, int x2) {
        ...
        for (int y = y1; y < y2; y++) { for (int x = x1; x < x2; x++) { output[y - y1][x - x1] = a[y][x]; } }
        return output;
    }
    ...
    public static double[][] concat(double[][] a, double[][] b, int dim) {
        if (dim == 0) {
            if (a[0].length == b[0].length) {
                ...
                for (int o = a.length; o < a.length + b.length; o++) { output[o] = makeCopy(b[o]); }
    ...
    public static double[][] concat(double[] a, double[][] b, int dim) { return flip(concat(flip(b, dim), a), dim); }
    ...
    public static double[][] concat(double[] a, double[] b, int dim) {
        if (dim == 0) {
            ...
            if (a.length == b.length) {
                ...
                for (int o = 0; o < a.length; o++) { output[o] = concat(a[o], b[o]); }
    ...
    public static double[] concat(double[] a, double[] b) {
        double[] output = new double[a.length + b.length];
        for (int o = 0; o < a.length; o++) { output[o] = a[o]; }
        for (int o = a.length; o < a.length + b.length; o++) { output[o] = b[o - a.length]; }
    ...
    public static double[] concat(double a, double[] b) { return flip(concat(flip(b), a)); } // might not be super fast
                                                                                             // but its okay

    public static double[] concat(double a, double b) { return new double[] { a, b }; } // redundant but hey thats what
                                                                                        // I'm going for

    public static int[] concat(int a, int[] b) {
        int[] output = new int[b.length + 1];

        output[0] = a;

        for (int o = 1; o < b.length + 1; o++) { output[o] = b[o - 1]; }

        return output;
    }

    public static double magnitude(double a) { return Math.abs(a); }

    public static double magnitude(double[] a) {
        double output = 0;
        for (int i = 0; i < a.length; i++) { output += a[i] * a[i]; }
    ...
    public static double[] magnitude(double[][] a) {
        ...
        for (int i = 0; i < a.length; i++) { output[i] = magnitude(a[i]); }
        return output;
    }

    public static double[] project(double[] a, double[] b) { return mult(dotProduct(a, b) / dotProduct(b, b), b); }
    ...
    public static double[] flip(double[] a) {
        double[] output = new double[a.length];
        for (int o = 0; o < a.length; o++) { output[o] = a[a.length - o - 1]; }
    ...
    public static double[][] flip(double[][] a, int dim) {
        if (dim == 1) {
            double[][] output = new double[a[0].length][];
            for (int o = 0; o < a.length; o++) { output[o] = flip(a[a.length - o - 1]); }
    ...
    public static double[][] transpose(double[][] a) {
        ...
        for (int i = 0; i < a.length; i++) { for (int j = 0; j < a[0].length; j++) { mat[j][i] = a[i][j]; } }
        return mat;
    }

    public static double[][] transpose(double[] a) { return new double[][] { makeCopy(a) }; }
    ...
    public static double[] concatenate(double... ds) { return ds; }
...
class ImproperDimensionException extends RuntimeException {
    ...
    public ImproperDimensionException() { super(BASIC); }

    // Constructor that accepts a message
    public ImproperDimensionException(String message) { super(BASIC + message); }

    // Constructor that accepts number(s)
    public ImproperDimensionException(double[] a) { super(BASIC + " a: " + a.length); }
    ...
    // Constructor that accepts number(s)
    public ImproperDimensionException(double[] a, double[] b) { super(BASIC + " a: " + a.length + ", b: " + b.length); }
