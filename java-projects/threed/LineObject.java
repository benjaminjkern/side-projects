...
public class LineObject extends Object3D {
    ...
    public double[] dirAndMag() { return Tools.subtract(end.pos, start.pos); }

    public double[] unitDir() { return Tools.unitVector(dirAndMag()); }

    @Override
    public void wireFrameDraw(Graphics g, LineObject camera, double fov, double stageWidth, double stageHeight) {
        g.setColor(color);
        g.drawLine(start.getScreenPos(camera, fov, stageWidth, stageHeight)[0],
                start.getScreenPos(camera, fov, stageWidth, stageHeight)[1],
                end.getScreenPos(camera, fov, stageWidth, stageHeight)[0],
                end.getScreenPos(camera, fov, stageWidth, stageHeight)[1]);
    }
