...
public class CellularAutomataFrame extends Keyframe {
    ...
    // border properties (Really only useful in the case of cellular automata, idk
    // why its in this class)
    ...
    CellularAutomataFrame(int width, int height, CellularAutomataFrame parent,
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                grid[y][x] = parent == null ? ((STARTRAND && Math.random() < randCount) ? 1 : 0)
                        : parent.doRule(x, y, ruleSet);
            }
        }
        border = 0;
    }
