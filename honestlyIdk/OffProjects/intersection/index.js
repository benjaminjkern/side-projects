    ...
    if (running) {
        ...
        while (goAgain && count < maxCount) {
            ...
            for (const node of savedNodes) {
                ...
                const possibleNewSpots = node.connections.map((connection, i) =>
                    setVecToDist(
                        nodes[connection].pos,
                        node.pos,
                        node.distances[i]
                    )
                );

                node.newPos = multConstVec(
                    1 / node.connections.length,
                    possibleNewSpots.reduce((p, c) => addVec(p, c), [0, 0])
                );
            }
            for (const node of savedNodes) {
...
const draw = () => {
    ...
    if (preSelectedNode) {
        ...
        ctx.beginPath();
        ctx.arc(
            preSelectedNode.pos[0],
            preSelectedNode.pos[1],
            selectRadius,
            0,
            2 * Math.PI
        );
...
window.onmouseup = (e) => {
    ...
    if (moving) {
        if (
            moving.pos[0] > canvas.width - 50 &&
            moving.pos[1] > canvas.height - 50
        ) {
            for (const node of safeNodes()) {
                node.connections = node.connections.filter(
                    (connection) => connection !== moving.id
                );
            }
    ...
    if (selectedNode && preSelectedNode) {
        if (selectedNode != preSelectedNode) {
            if (!selectedNode.connections.includes(preSelectedNode.id)) {
                ...
                selectedNode.connections = selectedNode.connections.filter(
                    (connection) => connection !== preSelectedNode.id
                );
                preSelectedNode.connections =
                    preSelectedNode.connections.filter(
                        (connection) => connection !== selectedNode.id
                    );
            }
        } else {
...
window.onkeydown = (e) => {
    if (e.key === " ") {
        ...
        if (running) {
            for (const node of safeNodes()) {
                node.v = [0, 0];
                node.distances = node.connections.map((connection) =>
                    vecDist(node.pos, nodes[connection].pos)
                );
            }
        }
    }
...
const setVecBothWaysToDist = (a, b, d) => {
    ...
    const sPlus = (1 + d / s) / 2;
    return [
        addVec(multConstVec(sPlus, a), multConstVec(sMinus, b)),
        addVec(multConstVec(sMinus, a), multConstVec(sPlus, b)),
    ];
};
